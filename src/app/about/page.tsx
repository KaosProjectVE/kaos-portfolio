...
export default function AboutPage() {
  const particlesOptions = {
    ...
    interactivity: {
      ...
      modes: { bubble: { distance: 100, size: 20, duration: 2 }, push: { quantity: 4 } },
    },
    detectRetina: true,
  }
  ...
  return (
    <main className="relative overflow-hidden bg-black text-white py-20 px-4">
      ...
      <div className="relative z-10 max-w-4xl mx-auto mb-20">
        <MotionH1 className="text-5xl font-bold text-center mb-8" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          Sobre Kaos Project
        </MotionH1>
        <MotionP className="text-center text-lg text-gray-200" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5, duration: 0.8 }}>
          En Kaos Project combinamos creatividad y tecnología para transformar ideas en experiencias visuales memorables. Nuestro equipo trabaja en cada detalle para que cada proyecto sea único.
        </MotionP>
      </div>
      ...
      <section className="relative z-10 max-w-4xl mx-auto mb-20">
        {timeline.map((item, i) => (
          <MotionDiv key={i} className="flex items-start mb-8" initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.2, duration: 0.6 }}>
            <div className="w-24 text-right pr-4 font-mono text-gray-300">{item.year}</div>
            <div className="flex-1 border-l border-gray-600 pl-6"><p className="text-lg text-gray-100">{item.event}</p></div>
          </MotionDiv>
        ))}
      </section>
      ...
      <section className="relative z-10 max-w-5xl mx-auto">
        <MotionH2 className="text-3xl font-bold mb-12 text-center" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
          Nuestro Equipo
        </MotionH2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {team.map((t, i) => (
            <MotionDiv key={i} className="bg-gray-800 rounded-2xl p-6 flex flex-col items-center" initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.3, type: 'spring', stiffness: 200 }}>
